...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    AuthLogger.logAttempt("login_attempt", "User attempting login", email, undefined, request);

    // Validate required fields
    if (["", null, undefined].includes(email)) {
      AuthLogger.logFailure("login_validation", "Email is required", email, undefined, request);
    ...
    if (["", null, undefined].includes(password)) {
      AuthLogger.logFailure("login_validation", "Password is required", email, undefined, request);
      return NextResponse.json(
    ...
    if (!!provider && !userPassword) {
      AuthLogger.logWarning("oauth_account_clash", `User tried credentials login but account exists with ${provider}`, email, undefined, request, { provider });
      return NextResponse.json(
        {
          success: false,
    ...
    AuthLogger.logSuccess("login_success", "User successfully logged in", email, undefined, request);
    return NextResponse.json(
    ...
    AuthLogger.logFailure("login_error", `Authentication failed: ${errorMessage}`, undefined, undefined, request);
    console.log(error);
    return NextResponse.json(
