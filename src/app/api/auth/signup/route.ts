...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (userPassword || provider) {
      ...
      if (provider && !userPassword) {
        ...
        AuthLogger.logWarning(
          "oauth_signup_conflict",
          `User tried to signup but OAuth account exists with ${provider}`,
          email,
          undefined,
          request,
          { provider }
    ...
    try {
      ...
      AuthLogger.logSuccess(
        "signup_complete",
        "User signup and auto-signin successful",
        email,
        addedUser.id,
        request
      );
      ...
      AuthLogger.logFailure(
        "signup_signin",
        `Failed to auto-signin after signup: ${errorMessage}`,
        email,
        addedUser.id,
        request
      );
    ...
    AuthLogger.logFailure(
      "signup_error",
      `Signup process failed: ${errorMessage}`,
      undefined,
      undefined,
      request
    );
